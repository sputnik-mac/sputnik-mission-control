...
let AGENTS = [
  ...
  { id: "finance", name: "Vault",   icon: "💰" },
  { id: "health",  name: "Doc",     icon: "🏥" },
];
...
async function startWatcher(broadcast, agentState) {
  ...
  try {
    // Use our own /api/agents endpoint which reads from openclaw config
    ...
    if (r.ok) {
      ...
      console.log(`[watcher] loaded ${AGENTS.length} agents:`, AGENTS.map(a => a.id).join(", "));
    }
  } catch (e) {
